"""PLC Trigger Recorder — main application window."""
...
# Thread-safe event queues (module-level so threads can import them)
...
# Maximum lines kept in the record log
_LOG_MAX_LINES = 500
...
class App(tk.Tk):
    def __init__(self) -> None:
        ...
        # Active recordings: save_path_str → display_label
    ...
    def _build_toolbar(self) -> None:
        ...
        # Simulation controls (hidden until sim mode enabled)
    ...
    def _toggle_simulation(self) -> None:
        ...
        if self._simulate_mode:
            ...
            self._sim_frame.pack_forget()
        # Restart monitor if already running
        if self._plc_monitor and self._plc_monitor.is_alive():
            self._plc_monitor.stop()
            self._start_plc_monitor()
    ...
    def _on_close(self) -> None:
        ...
        if self._recorder:
            self._recorder.stop()
        save_config(self._cfg)
        # Join with timeout — cv2 internal threads / saver threads may block
        if self._plc_monitor:
